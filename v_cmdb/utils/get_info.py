...
def get_device_info():
    ret = []
    for device, device_info in psutil.net_if_addrs().iteritems():  #python2
    #for device, device_info in psutil.net_if_addrs().items():   #python3
...
def get_disk_info():
    ...
    for dev in disk_data.stdout.readlines():
        #size = int(dev.strip().split()[4]) / 1024 / 1024/ 1024                   #python3
        size = int(dev.strip().split(',')[1].split()[0]) / 1024 / 1024/ 1024   #python2
...
def send(data):
    url = 'http://192.168.110.31:8001/cmdb/api_servers/'  #改成本机地址端口
